...
Once the Go button is pressed, the process starts running. It pre-processes the raw data and then matches against the databases such as Metlin, HMDB, KEGG and LipidMaps. The combination of *Preprocessing* and *Annotation* tabs is the same process as the *Autopipeline* tab with with the default value of the parameters (for more details refer to *Preprocessing* and *Annotation* tab documentation). 

The results of the *Autopipeline* are
...
## StatisticalAnalysis
The downstream analysis focuses on the significant identifications of metabolites for further analysis. The very first interest is to find the differentially expressed metabolites across all the groups. There are 3 collapsible panels; Differential Analysis, Network Analysis and Visualization. 
...
The Differential Analysis results in differential expression of the data. It provides 2 optionss  which are using Reference and Groups to chose the groups for comparsion of level of differential expressions. The reference can be selected and it will perform all the possible combinations of groups, keeping the chosen one as the control. The second option Groups provides all the possible combinations of the groups to choose one by one to customize the output.
